...
터미널 없이 **브라우저에서만** 할 수 있다. 사이트의 버튼이 내용이 미리 채워진
GitHub Issue 폼을 열어주고, 제출하면 Actions가 JSON과 Markdown을 대신 고쳐 커밋한다.

| 하고 싶은 것 | 어디서 |
|---|---|
| 오답노트 쓰기 | 문제 상세 → `오답노트 쓰기` |
| 복습 결과 남기기 | 복습 4단계 → `복습 결과 기록하기` |
| 배운 개념 추가 | 문제 상세 → `배운 개념 추가` |

폼을 제출하면 이슈가 자동으로 닫히고 1~2분 뒤 사이트에 반영된다.
커밋은 이슈를 올린 사람 이름으로 남으므로 잔디에도 그대로 찍힌다.
저장소 주인이 올린 이슈만 반영되니, 남이 연 이슈로는 데이터가 바뀌지 않는다.

### 터미널을 쓰고 싶다면

같은 일을 하는 CLI도 있다. 결과물은 폼과 동일하다.
...
문제 id는 `pgs-42627` 형식이다. CLI는 로컬 파일만 고치므로 `git push` 해야 사이트에 반영된다.
...
npm run import        정답 코드와 메타데이터를 아카이브로 가져옴 (이건 로컬에서)
   ↓
사이트에서 오답노트 쓰기 → Issue 폼 제출
   ↓
Actions 가 데이터 반영 + 커밋      학습 기록 자체가 Git 활동이 됨
   ↓
GitHub Pages 자동 배포
```
